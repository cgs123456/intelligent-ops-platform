...
class ClosedLoop:
    ...
    def get_status():
        ...
        for step_no, step_name, owner in ClosedLoop.STEPS:
            ...
            steps.append({
                'step': step_no, 'name': step_name, 'owner': owner,
            ...
            try:
                ...
                if step_no == 2:
                    ...
                    db.session.add(AuditLog(
                        actor=actor, action='loop_step_wait',
                        target_type='loop', target_id=str(run_id),
                ...
                db.session.commit()
                db.session.add(AuditLog(
                    actor=actor, action='loop_step_done',
                    target_type='loop', target_id=f'{run_id}-{step_no}',
                ...
                if rec:
                    ...
                    db.session.commit()
                db.session.add(AuditLog(
                    actor=actor, action='loop_step_timeout',
                    target_type='loop', target_id=f'{run_id}-{step_no}',
                    detail=str(e)[:500]))
                db.session.commit()
                # P1-2: 失败时触发告警通知
                ...
                if rec:
                    ...
                    db.session.commit()
                db.session.add(AuditLog(
                    actor=actor, action='loop_step_failed',
                    target_type='loop', target_id=f'{run_id}-{step_no}',
                    detail=str(e)[:500]))
                db.session.commit()
                # P1-2: 失败时触发告警通知
    ...
    def rollback_step(step_no, actor='system'):
        ...
        try:
            ...
            db.session.add(AuditLog(
                actor=actor, action='loop_step_rollback',
                target_type='loop', target_id=f'{run_id}-{step_no}',
                detail=f'回滚步骤{step_no}，原状态{old_status}，补偿 {len(compensations)} 项'
            ))
            db.session.commit()
            return {
                'step': step_no, 'status': 'rolled_back',
            ...
            db.session.add(AuditLog(
                actor=actor, action='loop_rollback_failed',
                target_type='loop', target_id=f'{run_id}-{step_no}',
                detail=f'回滚失败：{e}'
            ))
            db.session.commit()
    ...
    def _compensate_step3(run_id, actor):
        ...
        pos_to_cancel = PurchaseOrder.query.filter(
    ...
    def _compensate_step5(run_id, actor):
        ...
        step5_log = AuditLog.query.filter_by(
            action='loop_step_done', target_type='loop',
        ...
        for sug in approved:
            try:
                result = rpa.place_supplier_order({
                    'id': sug.id, 'product_id': sug.product_id,
    ...
    def _step5_fde_refresh(actor='system'):
        ...
        def _rows(v):
            if isinstance(v, dict):
                return v.get('rows', 0)
            return v
        return (f'ODS+{_rows(stats["ods"])} → DWD+{_rows(stats["dwd"])} → '
    ...
    def reset(actor='system'):
        ...
        db.session.commit()
        db.session.add(AuditLog(actor=actor, action='loop_reset',
                                target_type='loop', detail='闭环状态已重置'))
        db.session.commit()
        return '闭环已重置'
