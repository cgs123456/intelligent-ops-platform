...
class AuthService:
    ...
    def _encode(expires_delta, token_type, user):
        ...
        token = jwt.encode(full_payload, current_app.config['SECRET_KEY'],
    ...
    def login(username, password, ip=None):
        ...
        if not user or not user.password_hash or not check_password_hash(user.password_hash, password):
            db.session.add(AuditLog(
                actor=username, action='login_failed',
                target_type='auth', detail='密码错误',
                ip=ip,
            ))
            db.session.commit()
        ...
        # 3. 签发 access + refresh
        access_token, access_jti, _ = AuthService._encode(
            AuthService.ACCESS_TOKEN_EXPIRES, 'access', user)
        refresh_token, refresh_jti, _ = AuthService._encode(
            AuthService.REFRESH_TOKEN_EXPIRES, 'refresh', user)

        # 4. 登录成功审计
        db.session.add(AuditLog(
            actor=user.username, action='login_success',
            target_type='auth', target_id=str(user.id),
            detail=f'jti={access_jti[:8]}', ip=ip,
        ))
        db.session.commit()
    ...
    def refresh_token(refresh_tok, ip=None):
        ...
        try:
            payload = jwt.decode(
                refresh_tok, current_app.config['SECRET_KEY'],
            ...
            access_token, _, _ = AuthService._encode(
                AuthService.ACCESS_TOKEN_EXPIRES, 'access', user)
            ...
            db.session.add(TokenBlacklist(
                jti=jti, token_type='refresh', user_id=user.id,
                expires_at=datetime.fromtimestamp(payload['exp'], tz=timezone.utc),
            ))
            db.session.add(AuditLog(
                actor=user.username, action='token_refresh',
                target_type='auth', ip=ip,
    ...
    def logout(access_tok, refresh_tok=None, ip=None):
        ...
        for tok in [access_tok, refresh_tok]:
            ...
            try:
                payload = jwt.decode(
                    tok, current_app.config['SECRET_KEY'],
                    algorithms=[AuthService.ALGORITHM], options={'verify_exp': False}
                )
                jti = payload.get('jti')
        ...
        if revoked:
            db.session.add(AuditLog(
                ...
                action='logout', target_type='auth', ip=ip,
    ...
    def change_password(user_id, old_password, new_password, ip=None):
        ...
        db.session.add(AuditLog(
            actor=user.username, action='password_changed',
            target_type='auth', target_id=str(user.id), ip=ip,
        ))
        db.session.commit()
