...
class MultiAgentNegotiator:
    ...
    def negotiate(self, product, suppliers, suggested_qty=None):
        ...
        for sup in sup_list:
            try:
                ...
                logger.warning("[MultiAgent-Supplier] 报价生成失败 supplier=%s: %s", self._get(sup, "name", "?"), e)
        ...
        logger.info(
            "[MultiAgent] 博弈完成 product=%s quotes=%d best=%s",
            pname,
            len(scored),
    ...
    def _build_log(self, pname, buyer_demand, quotes, best):
        ...
        if best:
            lines.append(
                f"【决策】综合评分（价格50%+交期30%+评级20%）" f"选择 {best['supplier_name']}，得分 {best['score']}。"
            )
        else:
