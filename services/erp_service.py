...
class ERPService:
    ...
    def create_purchase_order(
        ...
        try:
            ...
            self._audit(
                ...
                f"product={product_id} qty={qty} price={unit_price}",
                ip=ip,
            )
            self.db.commit()
    ...
    def confirm_purchase_order(self, order_id, ext_order_no=None, actor="system", ip=None):
    ...
    def receive_purchase_order(self, order_id, actor="system", ip=None):
        ...
        try:
            ...
            self._audit(
                ...
                "purchase_order",
                order.id,
                detail=f"采购入库 {order.order_no} qty={order.qty} " f"avg_cost {old_avg}->{new_avg}",
                ip=ip,
            )
            self.db.commit()
    ...
    def cancel_purchase_order(self, order_id, actor="system", ip=None):
        ...
        try:
            ...
            self._audit(actor, "cancel", "purchase_order", order.id, detail=f"取消采购单 {order.order_no}", ip=ip)
            self.db.commit()
    ...
    def create_sale_order(
        self, product_id, qty, unit_price, customer, warehouse_id=None, platform="线下", actor="system", ip=None
    ):
        ...
        try:
            ...
            self._audit(
                ...
                f"qty={qty} receivable={receivable} cogs={cogs}",
                ip=ip,
            )
            self.db.commit()
            logger.info("销售单已创建: %s receivable=%s cogs=%s", order.order_no, receivable, cogs)
            return order
        except Exception as e:
            self.db.rollback()
    ...
    def create_return_order(
        self, original_order_no, product_id, qty, refund_amount, reason=None, actor="system", ip=None
    ):
        ...
        try:
            ...
            self._audit(
                ...
                f"qty={qty} refund={refund_amount} reason={reason}",
                ip=ip,
            )
            self.db.commit()
    ...
    def transfer_stock(self, product_id, qty, from_warehouse_id, to_warehouse_id, actor="system", ip=None):
        ...
        try:
            ...
            self._audit(
                ...
                f"ref={out_move.ref_order}",
                ip=ip,
            )
            self.db.commit()
            logger.info("库存调拨完成: %s -> %s qty=%s", from_warehouse_id, to_warehouse_id, qty)
    ...
    def get_recent_orders(self, limit=20):
        ...
        """
        orders = self.db.query(PurchaseOrder).order_by(PurchaseOrder.created_at.desc()).limit(limit).all()
        if not orders:
            return []
