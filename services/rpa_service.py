...
class RPAService:
    ...
    def _simulate_collect_quotes(self, today):
        ...
        try:
            ...
            if not products:
                recent_quotes = (
                    self.session.query(ExtSupplierQuote).order_by(ExtSupplierQuote.collected_at.desc()).limit(12).all()
                )
                seen_pids = set()
                products = []
            ...
            # 取所有启用供应商
            suppliers = self.session.query(Supplier).filter(Supplier.is_active.is_(True)).all()
    ...
    def _fallback_fluctuate_quotes(self, today):
        ...
        latest = ExtSupplierQuote.query.order_by(ExtSupplierQuote.collected_at.desc()).limit(12).all()
        for q in latest:
    ...
    def collect_supplier_quotes(self):
        ...
        self._audit(
            action="collect",
            target_type="ext_supplier_quote",
    ...
    def get_best_quote(self, product_id):
        ...
        """
        quotes = ExtSupplierQuote.query.filter_by(product_id=product_id, quote_date=date.today()).all()
        if not quotes:
            # 兜底：取该产品最近历史报价
            quotes = (
                ExtSupplierQuote.query.filter_by(product_id=product_id)
        ...
        for q in quotes:
            ...
            logger.debug(
                "[RPA-quote] 报价 %d 评分 %.2f（价格 %.2f + 交期 %d * %.1f）",
                q.id,
                score,
                float(q.quote_price),
                sup.lead_days,
        ...
        if best:
            logger.info(
                "[RPA-quote] 产品 %s 最优报价：供应商=%s 价格=%.2f 评分=%.2f",
                product_id,
                best[1].name,
                float(best[0].quote_price),
                best_score,
            )
        return best
    ...
    def _sync_single_order(self, ext_order):
        ...
        so = erp.create_sale_order(
            ...
            qty=ext_order.qty,
            unit_price=ext_order.unit_price,
            customer=ext_order.customer,
            platform=ext_order.platform,
            actor="rpa",
        )
    ...
    def sync_with_retry(self, ext_order_id, max_retry=None, backoff=None):
        ...
        for attempt in range(max_retry + 1):
            try:
                ...
                self.session.commit()
                logger.info("[RPA-sync] 订单 %s 同步成功（尝试 %d/%d）", ext_order_id, attempt + 1, max_retry + 1)
                return result
            except Exception as e:
                last_error = str(e)
                self.session.rollback()
                ...
                self.session.commit()
                logger.warning(
                    "[RPA-sync] 订单 %s 尝试 %d/%d 失败: %s", ext_order_id, attempt + 1, max_retry + 1, last_error
                )
        ...
        logger.error("[RPA-sync] 订单 %s 超过最大重试次数 %d，标记失败", ext_order_id, max_retry)
        self._audit(
            action="sync_fail",
            target_type="ext_ecommerce_order",
            target_id=ext_order_id,
            detail=f"订单同步失败，重试 {max_retry} 次：{last_error}",
        )
        return {"status": "fail", "reason": last_error, "ext_order_id": ext_order_id, "retries": max_retry + 1}
        ...
        for ext_order in unsynced:
            try:
                ...
                if ext_order:
                    ...
                    self.session.commit()
                results.append({"status": "fail", "reason": str(e), "ext_order_id": ext_order.id})
                fail_count += 1
    ...
    def place_supplier_order(self, suggestion):
        ...
        logger.info("[RPA-order] 自动下单开始：product_id=%s qty=%s", product_id, suggested_qty)

        best = self.get_best_quote(product_id)
        if not best:
            return {"status": "fail", "reason": "无可用供应商报价", "product_id": product_id}
        quote, supplier = best
        ...
        # 生成外部订单号（带随机后缀）
        ext_order_no = (
            f"EXT-{supplier.id}-" f'{datetime.now().strftime("%Y%m%d%H%M%S")}-' f"{random.randint(1000, 9999)}"
        )
        ...
        self._audit(
            action="order",
            target_type="erp_purchase_order",
            target_id=po.id,
            detail=(
                f"RPA 自动下单：供应商={supplier.name} 产品ID={product_id} "
                f"数量={suggested_qty} 单价={float(quote.quote_price)} "
                f"采购单={po.order_no} 外部单号={ext_order_no}"
            ),
        )
        logger.info("[RPA-order] 下单成功：po=%s ext=%s 金额=%.2f", po.order_no, ext_order_no, amount)
        return {
            "status": "ok",
            "po_id": po.id,
            "po_no": po.order_no,
    ...
    def run_scheduled_tasks(self):
        ...
        self._audit(
            action="schedule",
            target_type="rpa",
            detail=(f"定时任务执行：采集 {len(quote_result)} 条报价，" f"同步 {len(sync_result)} 条订单"),
        )
