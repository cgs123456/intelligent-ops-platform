...
class WarehouseService:
    ...
    def run_data_quality_tests(self):
        ...
        tests = [
            ...
            (DwsInventoryDaily, 'dws_inventory_snapshot_daily', ['product_id', 'dt', 'stock_qty'], None, 'stock_qty < 0'),
            (AdsReplenishmentSuggest, 'ads_replenishment_suggest', ['product_id', 'suggested_qty'], None, 'suggested_qty < 0'),
        ...
        for model, table, nn_cols, uniq_col, range_fail in tests:
            ...
            if range_fail:
                ...
                fail_count = self.session.query(func.count(model.id)).filter(
                    text(range_fail)
                ).scalar() or 0
    ...
    def get_ads_data(self):
        ...
        return {
            'suggestions': [{
                'id': s.id, 'product_id': s.product_id, 'product_name': s.product_name,
                'recent_7d_sales': s.recent_7d_sales, 'current_stock': s.current_stock,
                'in_transit': s.in_transit, 'suggested_qty': s.suggested_qty,
    ...
    def get_layer_stats(self):
        ...
        return {
            ...
            'ADS': {
                'ads_replenishment_suggest': self.session.query(AdsReplenishmentSuggest).filter_by(dt=date.today()).count(),
    ...
    def get_dq_report(self):
        ...
        rows = self.session.query(DataQualityLog).order_by(
            DataQualityLog.checked_at.desc()
        ).all()
        latest = {}
        for r in rows:
        ...
        try:
            ...
            for so in q.order_by(SaleOrder.created_at.asc()).all():
                ...
                if so.order_no in existing_nos:
                    continue
                self.session.add(OdsSaleOrder(
                    order_no=so.order_no, product_id=so.product_id, qty=so.qty,
                    unit_price=so.unit_price, customer=so.customer, platform=so.platform,
                    dt=so.created_at.date(), etl_batch=batch_no,
                ))
                n += 1
            total += n
            ...
            for po in q.order_by(PurchaseOrder.created_at.asc()).all():
                ...
                if po.order_no in existing_nos:
                    continue
                self.session.add(OdsPurchaseOrder(
                    order_no=po.order_no, supplier_id=po.supplier_id, product_id=po.product_id,
                    qty=po.qty, unit_price=po.unit_price, status=po.status,
                    dt=po.created_at.date(), etl_batch=batch_no,
                ))
                n += 1
            total += n
            ...
            existing_keys = {
                ...
                for r in self.session.query(
                    OdsStockMove.ref_order, OdsStockMove.product_id, OdsStockMove.qty,
                    OdsStockMove.move_type, OdsStockMove.dt
                ).all()
            }
            n, new_wm = 0, wm
            for sm in q.order_by(StockMove.created_at.asc()).all():
                ...
                if key in existing_keys:
                    continue
                self.session.add(OdsStockMove(
                    product_id=sm.product_id, qty=sm.qty, move_type=sm.move_type,
                    ref_order=sm.ref_order, dt=sm.created_at.date(), etl_batch=batch_no,
                ))
                n += 1
            total += n
        ...
        try:
            ...
            for ods in self.session.query(OdsSaleOrder).all():
                if ods.order_no in existing_sale:
                    continue
                self.session.add(DwdSalesFact(
                    order_no=ods.order_no, product_id=ods.product_id,
                    product_name=products.get(ods.product_id, '-'),
                    qty=ods.qty, amount=float(ods.qty or 0) * float(ods.unit_price or 0),
                    customer=ods.customer, platform=ods.platform, dt=ods.dt,
                ))
                n += 1
            total += n
            ...
            existing_stock = {
                (r[0], r[1], r[2], r[3])
                for r in self.session.query(
                    DwdStockFact.ref_order, DwdStockFact.dt,
                    DwdStockFact.move_type, DwdStockFact.product_id
                ).all()
            }
            n = 0
            for ods in self.session.query(OdsStockMove).all():
                ...
                if key in existing_stock:
                    continue
                self.session.add(DwdStockFact(
                    product_id=ods.product_id,
                    product_name=products.get(ods.product_id, '-'),
                    qty=ods.qty, move_type=ods.move_type,
                    ref_order=ods.ref_order, dt=ods.dt,
                ))
                n += 1
            total += n
    ...
    def _etl_dws(self, target_dates=None):
        ...
        try:
            ...
            for row in self.session.execute(sales_sql):
                ...
                if date_filter is not None and dt_val not in date_filter:
                    continue
                self.session.add(DwsSalesSkuDaily(
                    product_id=row[0], product_name=row[1], dt=dt_val,
                    sale_qty=int(row[3] or 0), sale_amount=float(row[4] or 0),
                ))
                sales_n += 1
            ...
            for d in snapshot_dates:
                for p in self.session.query(Product).all():
                    self.session.add(DwsInventoryDaily(
                        product_id=p.id, product_name=p.name,
                        dt=d, stock_qty=p.stock_qty,
                    ))
                    inv_n += 1
        ...
        self._update_etl_meta('DWS', 'dws_inventory_snapshot_daily', datetime.now(), details['dws_inventory_snapshot_daily'], 'success')
        ...
        return {'rows': details['dws_sales_sku_daily'] + details['dws_inventory_snapshot_daily'],
                'details': details}
    ...
    def _etl_ads(self, target_date=None):
        ...
        try:
            ...
            for p in products:
                # 近7日销量
                recent_sales = self.session.query(
                    func.coalesce(func.sum(DwsSalesSkuDaily.sale_qty), 0)
                ).filter(
                ...
                # 在途采购量（已确认未到货）
                in_transit = self.session.query(
                    func.coalesce(func.sum(PurchaseOrder.qty), 0)
                ).filter(
                ...
                reason = (f'近7日销量{recent_sales}件(日均{avg_daily:.1f})，当前库存{p.stock_qty}件，'
                          f'在途{in_transit}件，安全库存{p.safety_stock}件，'
                          f'交期{lead_days}天，建议补货{suggested}件。')

                self.session.add(AdsReplenishmentSuggest(
                    product_id=p.id, product_name=p.name,
                    recent_7d_sales=int(recent_sales), current_stock=p.stock_qty,
                    in_transit=int(in_transit), suggested_qty=suggested,
                    suggested_supplier_id=sup_id, suggested_supplier_name=sup_name,
                    reason=reason, dt=target_date,
                ))
                rep_n += 1
            ...
            total_sales = self.session.query(
                func.coalesce(func.sum(DwsSalesSkuDaily.sale_amount), 0)
            ).filter(
                DwsSalesSkuDaily.dt >= week_ago, DwsSalesSkuDaily.dt <= target_date,
            ).scalar() or 0

            total_qty = self.session.query(
                func.coalesce(func.sum(DwsSalesSkuDaily.sale_qty), 0)
            ).filter(
                DwsSalesSkuDaily.dt >= week_ago, DwsSalesSkuDaily.dt <= target_date,
            ).scalar() or 0

            total_purchase = self.session.query(
                func.coalesce(func.sum(PurchaseOrder.qty * PurchaseOrder.unit_price), 0)
            ).filter(PurchaseOrder.status.in_(['confirmed', 'received'])).scalar() or 0

            inventory_value = sum(float(p.cost_price or 0) * (p.stock_qty or 0) for p in products)

            top = self.session.query(
                DwsSalesSkuDaily.product_name,
                func.sum(DwsSalesSkuDaily.sale_qty).label('total'),
            ).filter(
                DwsSalesSkuDaily.dt >= week_ago, DwsSalesSkuDaily.dt <= target_date,
            ).group_by(DwsSalesSkuDaily.product_name).order_by(text('total DESC')).first()
            top_sku = top[0] if top else '-'

            low_count = sum(1 for p in products if (p.stock_qty or 0) < (p.safety_stock or 0))

            self.session.add(AdsDailyOpsReport(
                dt=target_date,
        ...
        self._update_etl_meta('ADS', 'ads_replenishment_suggest', datetime.now(), details['ads_replenishment_suggest'], 'success')
        ...
        return {'rows': details['ads_replenishment_suggest'] + details['ads_daily_ops_report'],
                'details': details, 'dt': str(target_date)}

    # ==================== 治理：血缘 / 元数据 / 数据质量 ====================

    def _record_lineage(self, upstream, downstream, layer, transformation):
        """记录上游→下游表血缘（幂等：已存在则跳过）"""
        exists = self.session.query(DataLineage).filter_by(
            upstream_table=upstream, downstream_table=downstream
        ).first()
        if exists:
            return
        self.session.add(DataLineage(
            upstream_table=upstream, downstream_table=downstream,
            layer=layer, transformation=transformation,
        ))
        try:
            self.session.commit()
        except Exception:
    ...
    def _init_lineage(self):
        ...
        existing = {
            (r[0], r[1])
            for r in self.session.query(
                DataLineage.upstream_table, DataLineage.downstream_table
            ).all()
        }
        added = False
        for up, down, layer, desc in self._LINEAGE_CHAINS:
            if (up, down) in existing:
                continue
            self.session.add(DataLineage(
                upstream_table=up, downstream_table=down,
                layer=layer, transformation=desc,
            ))
            added = True
        if added:
            self.session.commit()
    ...
    def _log_dq(self, table, test_name, column, status, failures):
        """写一条数据质量测试日志"""
        self.session.add(DataQualityLog(
            table_name=table, test_name=test_name, column_name=column,
            status=status, failures=int(failures or 0),
