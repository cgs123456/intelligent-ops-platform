...
class LoginSchema(Schema):
    ...
    username = fields.String(required=True, validate=validate.Length(min=1, max=64), description="用户名")
    password = fields.String(
        required=True, validate=validate.Length(min=1, max=128), description="密码", load_only=True
    )
...
class ChangePasswordSchema(Schema):
    ...
    old_password = fields.String(required=True, validate=validate.Length(min=1, max=128), load_only=True)
...
class CreateReturnSchema(Schema):
    ...
    original_order_no = fields.String(
        required=True, validate=validate.Length(min=1, max=64), description="原销售订单号"
    )
    product_id = fields.Integer(required=True, description="产品 ID")
    qty = fields.Integer(required=True, validate=validate.Range(min=1), description="退货数量（必须为正数）")
    reason = fields.String(required=False, validate=validate.Length(max=500), description="退货原因")
...
class CreateTransferSchema(Schema):
    ...
    product_id = fields.Integer(required=True, description="产品 ID")
    qty = fields.Integer(required=True, validate=validate.Range(min=1), description="调拨数量")
    from_warehouse = fields.String(required=True, validate=validate.Length(max=64), description="源仓库")
    to_warehouse = fields.String(required=True, validate=validate.Length(max=64), description="目标仓库")


class ReviewSuggestionSchema(Schema):
    """审核补货建议"""

    action = fields.String(required=True, validate=validate.OneOf(["approve", "reject"]), description="审核动作")
    final_qty = fields.Integer(required=False, validate=validate.Range(min=0), description="最终采购数量")
    note = fields.String(required=False, validate=validate.Length(max=500), description="审核备注")
...
class RunLoopStepSchema(Schema):
    ...
    step = fields.Integer(required=True, validate=validate.Range(min=1, max=5), description="步骤号 1-5")
